...
.admin-wrapper {
  ...
  grid-template-areas: "header" "nav" "maincontent";
}
...
.admin-add {
  ...
  &_bigest {
    ...
    &:hover {
      background-image: linear-gradient(to right, #005dff 0%, #0091ff 100%);
    }
  }
  &_middle {
    min-height: 380px;
...
.admin-add__icon {
  ...
  &::before {
    ...
    top: 50%;
    left: 50%;
    transform: rotate(45deg) translate(-65%,7%);
    background: svg-load('cross.svg', fill=#fff);
  }
}
.admin-btn {
  padding: 19px 45px;
